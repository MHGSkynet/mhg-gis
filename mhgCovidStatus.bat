...
::
::  Status Report application root folder.  This will likely need adjusting to wherever you put this app.
::
SET MHGGIS_ROOT=E:\gis\mhg-gis

::
::  QGIS install point root folder for default install. Adjust if you installed QGIS somewhere different
::
SET MHGGIS_QGIS_ROOT=C:\Program Files\QGIS 3.10

::
::  Location of python plug-ins for Python 3 default installation. Adjust if you installed python somewhere different.
::
set PYTHONPLUGS=%LOCALAPPDATA%\Programs\Python\Python38-32\Lib\site-packages
...
::
::  Let python know about QGIS libraries, and Google plug-ins
::
@echo off

:: set PYTHONPATH=%OSGEO4W_ROOT%\apps\qgis-ltr\python;%OSGEO4W_ROOT%\apps\qgis-ltr\python\plugins;%PYTHONPATH%

set PYTHONPATH=%OSGEO4W_ROOT%\apps\qgis-ltr\python;%PYTHONPATH%;%PYTHONPLUGS%
